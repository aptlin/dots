...
(bind-key "M-m" 'my/unfill-paragraph)
...
(setq org-default-notes-file "~/LOG/organizer.org")
...
" "Basic task data")
  (setq org-capture-templates
        `(("t" "Tasks" entry
           (file+headline "~/LOG/organizer.org" "Inbox")
          ...
          ("b" "sys.admin idea" entry
           (file+headline "~/code/sys.adm/tasks.org" "sys.adm")
           "* TODO %^{Task}"
           :immediate-finish t)
          ("E" "Energy" table-line
           (file+headline "~/LOG/organizer.org" "Track energy")
           "| %U | %^{Energy 5-awesome 3-fuzzy 1-zzz} | %^{Note} |"
           :immediate-finish t
           )
          ("e" "EDU" entry
           (file+headline "~/LOG/edu.org" "Tasks")
           ,my/org-basic-task-template)
          ("p" "People task" entry
           (file+headline "~/LOG/people.org" "Tasks")
           ,my/org-basic-task-template)
          ("j" "Journal entry" plain
           (file+datetree "~/LOG/journal.org")
           "%K - %a\n%i\n%?\n"
           :unnarrowed t)
          ("J" "Journal entry with date" plain
           (file+datetree+prompt "~/LOG/journal.org")
           "%K - %a\n%i\n%?\n"
           :unnarrowed t)
          ...
          ("c" "Protocol Link" entry (file+headline ,org-default-notes-file "Inbox")
           "* [[%:link][%:description]] \n\n#+BEGIN_QUOTE\n%i\n#+END_QUOTE\n\n%?\n\nCaptured: %U")
          ("db" "Done - Edu" entry
           (file+headline "~/LOG/business.org" "Tasks")
           "* DONE %^{Task}\nSCHEDULED: %^t\n%?")
          ...
          ("dt" "Done - Task" entry
           ...
           "* DONE %^{Task}\nSCHEDULED: %^t\n%?")
          ("q" "Quick note" item
           (file+headline "~/LOG/organizer.org" "Quick notes"))
          ("l" "Ledger entries")
          ("lm" "MBNA" plain
           (file "~/LOG/ledger")
           "%(org-read-date) %^{Payee}
  ...
  ")
          ("B" "Book" entry
           (file+datetree "~/LOG/books.org" "Inbox")
           "* %^{Title}  %^g
  %i
  *Author(s):* %^{Author} \\\\
  *ISBN:* %^{ISBN}
  ...
  *Review on:* %^t \\
  ...
  %U"
           :clock-in :clock-resume)
           ("C" "Contact" entry (file "~/LOG/contacts.org")
            "* %(org-contacts-template-name)
  :PROPERTIES:
  :EMAIL: %(my/org-contacts-template-email)
  :END:")
           ("n" "Daily note" table-line (file+olp "~/LOG/organizer.org" "Inbox")
            "| %u | %^{Note} |"
            :immediate-finish t)
           ("r" "Notes" entry
            (file+datetree "~/LOG/organizer.org")
            "* %?\n\n%i\n"
            )))
...
(defvar my/org-agenda-contexts
  ...
  (setq org-agenda-custom-commands
        `(("t" tags-todo "-cooking"
          ...
          ("b" todo ""
           ((org-agenda-files '("~/LOG/business.org"))))
          ("B" todo ""
           ((org-agenda-files '("~/Dropbox/books"))))
          ("o" todo ""
           ((org-agenda-files '("~/LOG/organizer.org"))))
          ("c" todo ""
           ((org-agenda-prefix-format "")
          ...
          ("C" "Cooking"
           ((tags "vegetables")
            ...
            (tags "pork")
            (tags "other"))
           ((org-agenda-files '("~/LOG/cooking.org"))
            (org-agenda-view-columns-initially t)
  ...
  (let (business relationships life business-next relationships-next life-next string emacs emacs-next
    ...
    (setq string
          (concat
           ...
           (format "  - *Coding* (%.1fh - %d%% of all)\n"
                   (my/quantified-get-hours "Discretionary - Productive - Coding" time-summary)
                   (/ (my/quantified-get-hours "Discretionary - Productive - Coding" time-summary) 1.68))
...
(defun my/org-add-line-item-task (task)
  ...
  (let ((org-capture-entry '("t" "Tasks" entry
                             (file+headline "~/LOG/organizer.org" "Tasks")
                             "")))
    (org-capture nil "t")
           ...
           (cond
            ...
            ((eq system-type 'gnu/linux)  ; Flickr
             (shell-command-to-string
              (if end
                  (format "cd /home/sacha/code/node; nodejs flickr-list.js -b %s -e %s" start end)
                (format "cd /home/sacha/code/node; nodejs flickr-list.js -b %s" start))))
            ((and nil  (eq system-type 'gnu/linux)) ;; Create links to sketches.sachachua.com/URL-encoded; not used at the moment
